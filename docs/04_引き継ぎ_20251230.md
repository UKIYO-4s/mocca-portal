...
| Phase | 内容 | 決定 | 理由 |
|-------|------|------|------|
| Phase 8 | Googleカレンダー連携 | ❌ 不要 | ポータル完結で代替（予約管理をポータル内で完結させるため外部カレンダー連携は不要） |
...
## 8. Phase 9 二要素認証(TOTP)モジュール

### 8.1 機能

| 機能 | 説明 |
|------|------|
| TOTP設定 | Google Authenticator/Authy等でQRコードスキャン |
| ログイン時認証 | 2FA有効ユーザーは6桁コード入力必須 |
| リカバリーコード | 8個の一回限り使用可能なコード生成 |
| 無効化 | パスワード確認後に2FAを無効化可能 |

### 8.2 データベース（usersテーブル追加カラム）

| カラム | 型 | 説明 |
|--------|-----|------|
| two_factor_secret | varchar(100) | 暗号化されたTOTPシークレット |
| two_factor_recovery_codes | text | 暗号化されたリカバリーコード配列 |
| two_factor_confirmed_at | timestamp | 2FA有効化日時 |

### 8.3 画面

| 画面 | 対象 | 機能 |
|------|------|------|
| Setup.tsx | 全員 | 2FA設定画面（QRコード表示・有効化・無効化） |
| Challenge.tsx | 2FA有効ユーザー | ログイン時の認証コード入力 |
| Profile/Edit.tsx | 全員 | 2FA設定へのリンク追加 |
...
database/migrations/
└── 2025_12_29_153406_add_two_factor_columns_to_users_table.php
...
| 項目 | 内容 |
|------|------|
| QRコード生成 | BaconQrCode（サーバーサイドSVG生成、Google Charts API不使用） |
...
├── Http/
│   ├── Controllers/Auth/
│   │   └── TwoFactorController.php  # 二要素認証 ✅ 完了
│   └── Middleware/
│       └── EnsureTwoFactorAuthenticated.php  # 2FA検証 ✅ 完了
├── Modules/                      # 機能モジュール
...
| Phase | 内容 | 状態 |
|-------|------|------|
| Phase 1 | コア基盤（認証・権限） | ✅ 完了 |
| Phase 2 | コア拡張 + 予約モジュール | ✅ 完了 |
| Phase C | JPYC投げ銭システム | ✅ 完了 |
| Phase 3 | チェックリストモジュール | ✅ 完了 |
| Phase 4 | 備品管理モジュール | ✅ 完了 |
| Phase 5 | タイムカードモジュール | ✅ 完了 |
| Phase 6 | お知らせモジュール | ✅ 完了 |
| Phase 7 | シフト管理モジュール | ✅ 完了 |
| Phase 9 | 二要素認証(TOTP) | ✅ 完了 |
| Phase 8 | Googleカレンダー連携 | ❌ 不要と決定 |

### 今後の検討事項
1. 各モジュールのユーザーフィードバック収集・改善
2. パフォーマンス最適化
3. 追加機能要望への対応

---

**全Phaseの実装・レビュー完了 - 引き継ぎ完了**
